...
from AlphaTwirl.ConcurrentlySP import CommunicationChannel, TaskPackage
...
class TestCommunicationChannel(unittest.TestCase):
    ...
    def test_put(self):
        ...
        self.assertEqual([
            TaskPackage(task = task1, progressReporter = None, args = (), kwargs = {}),
            TaskPackage(task = task2, progressReporter = None, args = (123, 'ABC'), kwargs = {'A': 34}),
        ], dropbox.packages)
    ...
    def test_put_progressMonitor(self):
        dropbox = MockDropbox()

        progressMonitor = MockProgressMonitor()
        obj = CommunicationChannel(
            dropbox = dropbox,
        ...
        self.assertEqual([
            TaskPackage(
                task = task1,
                progressReporter = progressMonitor.reporters[0],
                args = (), kwargs = {}
            ),
            TaskPackage(
                task = task2,
                progressReporter = progressMonitor.reporters[1],
                args = (123, 'ABC'), kwargs = {'A': 34}
            ),
        ], dropbox.packages)
