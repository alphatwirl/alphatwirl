...
class TestScan(unittest.TestCase):
    ...
    def test_init_contents(self):
        ...
        np.testing.assert_equal(np.array([[10, 20], [30, 40]]), obj.contents)

    def test_init_None(self):
        obj = Scan()
        np.testing.assert_equal(np.array([]), obj.contents)

    def test_add(self):
        obj1 = Scan(contents = [[10, 20], [30, 40]])
        obj2 = Scan(contents = [[50, 60], [70, 80]])
        obj3 =  obj1 + obj2
        np.testing.assert_equal(np.array([[10, 20], [30, 40], [50, 60], [70, 80]]), obj3.contents)
