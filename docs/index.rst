...
for one event; for example, data in `ROOT <https://root.cern.ch/>`
`TTrees <https://root.cern.ch/doc/master/classTTree.html>` are event data when
...
`R <https://www.r-project.org/>` or `pandas <http://pandas.pydata.org/>` as data
frames. Then, users can continue a multi-dimensional categorical analysis with
